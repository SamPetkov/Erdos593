...
/-- Explicit, pairwise-distinct third vertices together with their host-edge
witnesses.  `Nonempty (PairCodegreeWitness H x y t)` is the formal
codegree-at-least-`t` assertion used by the positive-atom construction. -/
...
variable {W : Type u} {D : Type v} {H : TripleSystem W D}
  {x y : W} {t : Nat} (p : PairCodegreeWitness H x y t)

def thirdWitness (k : Fin t) : ThirdVertexWitness H x y (p.third k) where
  edge := p.edge k
  edgeSet_eq := p.edgeSet_eq k

def swap : PairCodegreeWitness H y x t where
  third := p.third
  edge := p.edge
...
theorem third_ne_left (k : Fin t) : p.third k ≠ x :=
  ThirdVertexWitness.third_ne_left (thirdWitness p k)

theorem third_ne_right (k : Fin t) : p.third k ≠ y :=
