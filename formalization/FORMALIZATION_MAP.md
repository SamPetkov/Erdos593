...
| N5 | `SequenceLift.baseFiberAssemblyCompatible`, `SequenceLift.baseFiberAssemblyCompatible.runningEdgeAssembly`, `SequenceLift.edgeRestriction_constructible_of_linear_of_hostColorable_of_baseFiberAssembly`, `SequenceLift.edgeRestriction_isObligatory_of_linear_of_hostColorable_of_baseFiberAssembly` | an explicitly ordered finite family of linear base fibres over a two-colourable host can be glued to an exact constructible, hence obligatory, restriction when it covers the selected edge set and satisfies the literal running-intersection condition: each new fibre is edge-disjoint from the accumulated tail and has disjoint or singleton-intersecting total vertex support. This does not derive that support geometry from the existing edge-fibre partition. |
