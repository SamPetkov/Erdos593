...
| N3 | `SequenceLift.baseFiber_constructible_of_linear_of_colorable` | a finite linear base fibre is constructible provided its canonical base-letter subgraph is two-colourable. The colourability assumption is essential and explicit: linearity alone supplies no bipartiteness. This remains fibre-local and does not yield a global decomposition or constructibility theorem for the whole selected family. |
| N4 | `SequenceLift.baseFiber_isObligatory_of_linear_of_colorable` | the same explicitly two-colourable finite linear base fibre is obligatory, by N3 and the completed classical theorem that constructible systems are obligatory. It is a local use of the positive-atom theorem, not a global sequence-lift atom or trace result. |
