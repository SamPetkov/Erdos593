...
identified with its precise source-edge index subtype. These are local
finite-trace bridges, not yet a global finite-trace decomposition. The project is not yet a complete machine-checked
...
`SequenceLiftBaseFiber` canonical trace-key and fibre modules on every pull request and
...
It contains 13,541 physical lines, 87 source modules, and 43 external Mathlib
