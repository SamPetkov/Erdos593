...
that every other fibre is singleton-or-empty. It is
...
`SequenceLiftBaseNode` / `SequenceLiftBaseNormalForm` canonical-base-node
modules on every pull request and push to `main`. The workflow deliberately
...
It contains 13,129 physical lines, 84 source modules, and 43 external Mathlib
