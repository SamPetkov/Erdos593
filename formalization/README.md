...
one key per edge. This is a local finite-trace bridge, not yet a global
finite-trace decomposition. The project is not yet a complete machine-checked
...
`SequenceLiftBaseLetter` / `SequenceLiftFiniteTrace` canonical trace-key
modules on every pull request and
...
It contains 13,418 physical lines, 86 source modules, and 43 external Mathlib
