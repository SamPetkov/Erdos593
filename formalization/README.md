...
subtype cardinal gives the same count. These are
local finite-trace bridges, not yet a global finite-trace decomposition. The project is not yet a complete machine-checked
...
`SequenceLiftBaseFiber` / `SequenceLiftBaseFiberIndex` canonical trace-key and
fibre modules on every pull request and
...
It contains 13,672 physical lines, 88 source modules, and 43 external Mathlib
