...
linear-trace rigidity package. The new `SequenceLiftBaseNode` module supplies
the canonical grouping index needed for the next trace step: `BasedAt q e`
means that `q` contains two distinct points of `e`, `basedAt_unique` proves
that such a node is unique, `baseNode e` selects it classically, and
`baseNode_mkEdge` identifies the selector on every displayed lift edge. It is
not yet a complete machine-checked proof of Erdős Problem 593: the full
...
`SequenceLiftBaseNode` canonical-base-node module on every pull request and
push to `main`. The workflow deliberately avoids the aggregate default target;
...
It contains 13,028 physical lines, 83 source modules, and 43 external Mathlib
