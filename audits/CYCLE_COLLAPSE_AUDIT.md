...
\sigma\lhd\tau,
\qquad
B(\sigma,a)=\{(\sigma,x):x\in a\},
\qquad |a|=s,
\]

with \(a=\tau(\operatorname{lh}\sigma)\).
...
or one of \(
u_i,
u_{i+1}\) is a proper prefix of the other.
...
Choose \(j\) such that \(
u_j\) has minimum ordinal length. Rotate the cycle so that \(j=0\), and put

\[
\sigma=\nu_0,
\qquad
d=\operatorname{lh}(\sigma).
\]
...
Assume \(\sigma\preccurlyeq\nu_i\). If \(
u_i\preccurlyeq\nu_{i+1}\), the claim is immediate. Otherwise
...
Both \(\sigma\) and \(
u_{i+1}\) are prefixes of \(
u_i\), and minimum length gives

\[
d\le\operatorname{lh}(\nu_{i+1}).
\]
...
In particular, every connector of length \(d\) is equal to \(\sigma\).
...
Suppose some connector node is strictly above \(\sigma\). After a second cyclic rotation if necessary, choose a maximal nonempty interval
...
The equality \(
u_{t+1}=\sigma\) is an equality of node values at a boundary connector occurrence. If the minimum node occurs only once on the cycle, then \(t=m-1\) and the two boundary occurrences are the same connector occurrence \(v_0\), approached from opposite sides.

The two boundary hyperedges are nevertheless

\[
e_0\quad\text{and}\quad e_t=e_{m-1},
\]

which are distinct because Berge-cycle edge indices are distinct and \(m\ge3\).
...
This is defined because \(\operatorname{lh}(\nu_i)>d\). Consecutive nodes \(
u_i,
u_{i+1}\) are comparable and both extend \(\sigma\), so they agree at coordinate \(d\). Therefore

\[
a_1=a_2=\cdots=a_t=:a.
\]

**Audit status:** passed. Comparability, not merely common extension of \(\sigma\), is the key input.

### C5. The two boundary edges contain the same complete base

The edge \(e_0\) has connectors at the two distinct nodes

\[
\sigma\quad\text{and}\quad\nu_1.
\]

Since \(\sigma\lhd\nu_1\), these must be respectively the base and apex nodes of the lifted image of \(e_0\). Its base-edge index is
...
\[
B=B(\sigma,a)=\{(\sigma,x):x\in a\}.
\]

Likewise, \(e_t\) has connectors at \(
u_t\) and \(\sigma\), with \(\sigma\lhd\nu_t\), so its target image contains the same complete base \(B\), because
...
Therefore no connector lies strictly above \(\sigma\), and all connector nodes equal \(\sigma\).
...
| Infinite \(\kappa\) | Not used here | It is needed for the chromatic construction, not for collapse. |
...
Their connector nodes are

\[
(a),\quad\varnothing,\quad(a),
\]

so collapse fails. Precisely as predicted by the proof, the two boundary edges \(E_0,E_1\) share the full two-point base \(B(\varnothing,a)\), so the source is not linear.
