...
\begin{abstract}
We classify finite obligatory triple systems and formalise the finite
structural core of the classification in Lean~4.  After isolated vertices
are deleted, a triple system is obligatory exactly when it is linear, every
hyperedge-node of its Levi graph is incident with a bridge, and every Berge
cycle has even length.  The associated bridge decomposition yields a
canonical forest of atoms, each of which is either one triple or the
private-vertex expansion of a finite $2$-connected bipartite graph.  This
normal form determines the exact order--size--component spectrum, together
...
The present proof and Lean implementation were developed independently, in
parallel with closely related work by Li.  Li's preprint, posted on 23 June
2026, gives the same classification \citep[Theorem~1.1]{li2026} and an
alternative complete-rank one-apex lift and bridge-trace formulation
\citep[Sections~3--4 and Theorem~4.6]{li2026}.  We cite this work for
chronology and comparison; none of its results is used below as a black box.
Our approach supplies direct positive arguments, a separate
...
in ZFC.  The only imported results used as black boxes are the de Bruijn--Erdős
...
nonempty.  We first identify each fibre with an expansion atom and then show
...
compactness, the classical Erd\H{o}s--Hajnal high-odd-girth theorem, and the
standard Erd\H{o}s--Rado relation (8.1).  It assumes neither CH nor GCH and
...
The public commit history records a checked formalisation scaffold dated
\href{https://github.com/SamPetkov/Erdos593/commit/ba37b8c511ab390c14a905110366d7cac1ffa08f}{15 July 2026}
and, later that day, the complete finite structural classification in
...
formalisation.  The formalisation supplements rather than replaces
mathematical review.  Its exported endpoint covers Theorem~A and the
isolated-vertex reduction.  The manuscript proves the canonical-atom normal
...
restructuring, and the Lean formalisation.  The author reviewed every
incorporated suggestion and takes full responsibility for the arguments,
...
\newblock \emph{Proc. Amer. Math. Soc.}, 2024.
